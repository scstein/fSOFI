function [sof, win_sof] = FSOFI_Analysis(im, ncum, win_size, time_lags, itp_factor, mirrorMode, show_intermediates)
...
% Note: You can use 
%           imagesc(sof); colormap hot; axis image;
%       to visualize the result.
%
% Input
% im - 3D (x,y,time) image stack
...
%              dimension [ipX,ipY]. If a single number is given, the same
%              factor is used for all dimensions. The output is of size
%              itp_fac.*size(img)| default: 1
% mirrorMode - Specifies wether to use periodic mirroring of the input data before
...
%          specific dimension. Possible values: 'none','lateral' | default: 'none'
% show_intermediates - show intermediate results | default: false
%
% Output
% sof(xdim, ydim) - SOFI result image
% win_sof(xdim,ydim,nr_windows) - SOFI result for every window
%
...
if nargin <3 || isempty(win_size)
    win_size = min( size(im,3), 100);
else
    win_size = min( size(im,3), win_size);
end
...
if nargin <7 || isempty(show_intermediates)
...
% Memory allocation
win_nr = floor(size(im,3)/win_size); % Number of windows
ip_im = zeros( itp_factor(1)*size(im,1), itp_factor(2)*size(im,2), win_size); % Storage for interpolated images
win_sof = zeros(size(ip_im,1),size(ip_im,2), win_nr); % Result for each individual window
sof = zeros(size(ip_im,1),size(ip_im,2));             % Final SOFI result
...
try
    for iWin = 1:win_nr
        ...
        if(max(itp_factor) > 1)
            ...
            win_sof(:,:,iWin) = autoCumulant2D_cpp(ip_im,ncum, time_lags); % C++ (recommended)
            %             win_sof(:,:,iWin) = autoCumulant2D_MATLAB(ip_im,ncum, time_lags); % MATLAB
            sof = sof + win_sof(:,:,iWin);
            rewPrintf('done\n');
        else
            %--- Core SOFI Computation ---
            rewPrintf('  Computing the cumulant.. ');
            win_sof(:,:,iWin) = autoCumulant2D_cpp(double(im(:,:,min_frame:max_frame)),ncum, time_lags); % C++ (recommended)
            %             win_sof(:,:,iWin) = autoCumulant2D_MATLAB(double(im(:,:,min_frame:max_frame)),ncum, time_lags); % MATLAB
            sof = sof + win_sof(:,:,iWin);
            rewPrintf('done\n');
        end
        
        if show_intermediates
            imagesc(abs(sof)); colormap hot; axis image; title(sprintf('Window %i/%i \n', iWin, win_nr')); drawnow;
        end
    end
    sof = sof/win_nr;    
catch Merr
    warning('Error during FSOFI_Analysis. Did you install the ''Microsoft Visual Studio 2012 Redistributable Version 4'' for the MEX functions to work? If not, please install (recommended) or uncomment the MATLAB version of the cumulant calculation inside the code (not recommended).');
    throw(Merr);
end
